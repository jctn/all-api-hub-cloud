...
describe("CheckinOrchestrator", () => {
  ...
  it("uses a browser-session fallback for manual Ouu check-ins before trying refresh", async () => {
    const ouuAccount: SiteAccount = {
      ...baseAccount,
      site_name: "OuuAPI",
      ...
      authType: AuthType.Cookie,
      cookieAuth: {
        sessionCookie: "sid=ouu-cookie",
      },
    }
    const repository = await createRepositoryWithAccounts([ouuAccount])
    let browserFallbackCalls = 0
    let refreshCalls = 0
    ...
    const refresher: SiteSessionRefresher = {
      async refreshSiteSession(): Promise<SessionRefreshResult> {
        ...
        return {
          status: "failed",
          message: "should not be called for ouu browser fallback",
        }
      },
      async checkInWithBrowserSession(): Promise<CheckinAccountResult> {
        browserFallbackCalls += 1
        const now = Date.now()
        return {
          accountId: ouuAccount.id,
          siteName: ouuAccount.site_name,
          siteUrl: ouuAccount.site_url,
          siteType: ouuAccount.site_type,
          status: CheckinResultStatus.Success,
          message: "签到成功；已通过浏览器会话补签",
          startedAt: now,
          completedAt: now,
