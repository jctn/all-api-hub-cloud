...
import type { FlareSolverrResult } from "../src/auth/flareSolverrClient.js"
import {
...
describe("PlaywrightSiteSessionService", () => {
  ...
  it("fetches /api/status before rendering runanytime turnstile when the page needs site config", async () => {
    const service = new PlaywrightSiteSessionService(
      {} as StorageRepository,
      baseConfig,
      async () => {
  ...
  it("uses a visible runanytime turnstile widget flow instead of invisible execute mode", async () => {
    const service = new PlaywrightSiteSessionService(
      {} as StorageRepository,
      baseConfig,
      async () => {
    ...
    const page = {
      async evaluate(fn: unknown, arg?: unknown) {
        ...
        const source = String(fn)
        if (
          source.includes("turnstile.render") &&
          !source.includes('size: "invisible"') &&
          !source.includes("turnstile.execute")
        ) {
          return {
            statusCode: 200,
            rawText: JSON.stringify({
  ...
  it.skip("reuses the runanytime root session when the root page is already authenticated", async () => {
    ...
    const capturedAccount: SiteAccount = {
      ...baseAccount,
      site_name: "随时跑路公益站",
      site_url: "https://runanytime.hxi.me",
      account_info: {
    ...
    try {
      ...
      const completeLoginFlow = vi.fn(async () => {
        throw new Error("should not continue the login flow when the root session is valid")
      })
      const performBrowserSessionCheckin = vi.fn(async (_page, account: SiteAccount) => ({
        status: CheckinResultStatus.Success,
        message: account.account_info.access_token,
      }))
      ...
      ).captureAuthenticatedAccount = async () => capturedAccount
      ;(
        service as unknown as {
          completeLoginFlow: typeof completeLoginFlow
          performBrowserSessionCheckin: typeof performBrowserSessionCheckin
      ...
      ).performBrowserSessionCheckin = performBrowserSessionCheckin
      ...
      expect(result?.status).toBe(CheckinResultStatus.Success)
      expect(gotoCalls).toEqual(["https://runanytime.hxi.me/"])
      expect(savedAccounts).toEqual([capturedAccount])
      expect(completeLoginFlow).not.toHaveBeenCalled()
      expect(performBrowserSessionCheckin).toHaveBeenCalledTimes(1)
      expect(performBrowserSessionCheckin.mock.calls[0]?.[1]).toEqual(capturedAccount)
    } finally {
      launchSpy.mockRestore()
    }
  })
  ...
  it.skip("allows manual fallback when runanytime explicitly sets manualFallbackPolicy to last-resort", async () => {
    ...
    try {
      ...
      const waitForManualLoginCompletion = vi.fn(async () => page)
      const performBrowserSessionCheckin = vi.fn(async () => ({
        status: CheckinResultStatus.Success,
        message: "ok",
      }))
      ;(
  ...
  ;[
    ...
    {
      label: "local flaresolverr url is missing",
      localFlareSolverr: {
        enabled: true,
        url: null,
        timeoutMs: 90_000,
      ...
      try {
        const service = new PlaywrightSiteSessionService(
          {} as StorageRepository,
          {
            ...baseConfig,
            browserHeadless: false,
            flareSolverrUrl: "https://cloud.example.com",
            localFlareSolverr,
            siteLoginProfiles: {
              "demo.example.com": {
                hostname: "demo.example.com",
        ...
        expect(result?.status).toBe(CheckinResultStatus.Failed)
        expect(result?.code).toBe("local_flaresolverr_unavailable")
        expect(result?.message).toBe("本地 FlareSolverr 不可用，无法执行预热模式")
        expect(launchSpy).not.toHaveBeenCalled()
        expect(progress).toContain("命中本地 FlareSolverr 预热策略，但本地能力不可用")
      } finally {
        launchSpy.mockRestore()
      }
    })
  ...
  it.skip("prewarms local-browser sessions with flaresolverr before opening the site flow", async () => {
    ...
    const launchSpy = vi
      ...
      .mockImplementation(async (_userDataDir, options) => {
        timeline.push(`launch:${String(options.userAgent ?? "")}`)
    ...
    try {
      ...
      expect(progress).toContain("命中本地 FlareSolverr 预热策略")
      expect(timeline[0]).toBe("prewarm-fetch")
      expect(timeline[1]).toBe("launch:ua-local-prewarm")
      expect(launchSpy).toHaveBeenCalledWith(
        baseConfig.sharedSsoProfileDirectory,
  ...
  it("stops retrying after one FlareSolverr attempt when Linux.do callback challenge still does not transition", async () => {
    const progress: string[] = []
    ...
    const runtime = service as unknown as {
      ...
      completeLoginFlow: (
        ...
        profile: SiteLoginProfile,
        options: { onProgress?: (message: string) => void | Promise<void> },
      ) => Promise<{ status: string; page?: Page; message?: string }>
    }
